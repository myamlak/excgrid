...
XcKernelValue PbeExchange(
    ...
    const double C259 = C258 / 0.804;
    const double C260 = C259 + 1.;

    result.vrhoA = -((1.804 - 0.804 / C260) * (rhoA * 3. * C255 / 3. + C246) +
                     rhoA * C246 * -0.705960152410687 * (2. * C240 + 2. * rhoA * C255 / 3.) * C247 /
                         (1.608 * C251 * std::pow(C254, 2) * std::pow(C260, 2))) /
                   (4. * Pi);
    ...
    const double C275 = C274 / 0.804;
    const double C276 = C275 + 1.;

    result.vrhoB = -((1.804 - 0.804 / C276) * (rhoB * 3. * C271 / 3. + C262) +
                     rhoB * C262 * -0.705960152410687 * (2. * C241 + 2. * rhoB * C271 / 3.) * C263 /
                         (1.608 * C267 * std::pow(C270, 2) * std::pow(C276, 2))) /
                   (4. * Pi);
    ...
    result.vsigmaAa =
        -4.2357609144641219 * C283 * rhoA /
        (0.25728e2 * Pi *
         std::pow(0.2195149727645171 * std::pow(std::sqrt(4. * sigmaAa) / (2. * C285), 2) / 0.804 +
                      1.,
                  2) *
         std::pow(C285, 2));
    ...
    result.vsigmaBb =
        -4.2357609144641219 * C293 * rhoB /
        (0.25728e2 * Pi *
         std::pow(0.2195149727645171 * std::pow(std::sqrt(4. * sigmaBb) / (2. * C295), 2) / 0.804 +
                      1.,
                  2) *
         std::pow(C295, 2));
