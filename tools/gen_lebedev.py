...
each) is a published property of the quadrature and is taken as such, and
the parameters are seeded from the published values.  Both are published
MATHEMATICAL DATA - the numbers every correct implementation of this
quadrature carries - read as read-only input from a published reference
implementation's tables, not from its code; no reference source code is
shipped here, linked, or used at build or run time.  The seeded values are
never modified directly: they are refined by Gauss-Newton in high
precision (mpmath, 60 digits).  The solve therefore both VERIFIES the
published values - a correct table converges to a residual near the solve
precision - and corrects them when they are wrong; the certificate
(max |moment residual|) is printed per size and embedded in the generated
header.
...
def emit_header(certified: dict[int, list[tuple[int, list[mpmath.mpf]]]]) -> str:
    ...
    add("// The reference tables are NOT shipped with this repository, so this header")
    add("// cannot be re-derived from a checkout alone; the generator prints a")
    add("// max |moment residual| per size, which the committed table satisfies.")
    add("//")
    add("// Every grid below was SOLVED for from the moment conditions")
    add("//     sum_i w_i m(p_i) = int_{S^2} m dOmega")
    add("// for all monomials x^A y^B with A, B even, A + B <= degree.  Weights are")
    add("// normalized so that sum w_i = 4 pi (int 1 dOmega).")
    add("//")
    add("// PROVENANCE.  The orbit structure per size and the parameter values used")
    add("// as the solve seed are PUBLISHED MATHEMATICAL DATA - the numbers every")
    add("// correct implementation of this quadrature carries - read as read-only")
    add("// input from a published reference implementation's tables, not from its")
    add("// code.  No reference source code is shipped with excgrid, linked into the")
    add("// library, or used at build or run time; the tables below are re-derived")
    add("// from the moment conditions and re-certified against them.")
    add("#pragma once")
    add("")
    add("#include <array>")
    add("#include <cstddef>")
