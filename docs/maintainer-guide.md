...
For a literature functional, read the energy-density formula from a reference
implementation or the published paper. For a novel functional, the
...
- **The Libxc point check** (`tools/verify_pyscf.py`) for ported functionals.
...
7. `tools/verify_pyscf.py` — its `FUNCTIONALS` table, one entry per compared
   kernel. **A ported addition missing here has no point-check coverage and
   nothing says so**: the run stays green for the kernels it does compare. A
   novel functional has no Libxc id to compare against and is covered by the
   finite-difference channel instead, which is why this item is on the ported
   path.
8. `docs/kernel-api.md` §4 — the name list in the shipped set.
9. `CHANGELOG.md` — the kernel count in the unreleased entry.
10. `xc_defs/freshness.json` — rewritten by step 3; commit it.
11. `docs/maintainer-guide.md` — this list, if the addition taught you
    something it does not already say.
12. Then the closing checks: `python tools/check_freshness.py` fast and green,
...
(octahedral symmetry + polynomial exactness). The solve runs in 60-digit
arithmetic (mpmath) and prints a per-size certificate — max |moment residual|
at the reference positions — which is a statement about the *solve*, not about
the shipped table: the committed values are the double-precision rounding of a
