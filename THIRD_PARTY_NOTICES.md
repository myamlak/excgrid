...
| Lebedev-Laikov quadrature tables | 16 sizes, re-derived and certified | angular quadrature (`include/excgrid/internal/lebedev_tables.hpp`) | published mathematical data; tables re-derived and certified by `tools/gen_lebedev.py` from the moment conditions, and carrying the citation to Lebedev & Laikov, Dokl. Math. 59 (1999) 477 that `CITATION.bib` records. Only the published numerical data was used (the orbit structure per size and the values as a solve seed); no reference source code is shipped, linked, or used at build or run time |
